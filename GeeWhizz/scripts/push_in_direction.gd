...
func _ready():
	...
	self.set_applied_torque(rotation_speed * 200)
	...
	if Engine.is_editor_hint():
		if direction != Vector2():
			...
			if self.has_node("Sprite"):
				...
				draw_set_transform((direction) * Vector2(speed, speed), deg2rad(sprite_rotation), scale)
...
func set_sprite_rotation(value):
	...
	if self.has_node("Sprite"):
		self.get_node("Sprite").rotation = deg2rad(sprite_rotation)
		self.get_node("CollisionShape2D").rotation = deg2rad(sprite_rotation)
